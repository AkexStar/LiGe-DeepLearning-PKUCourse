...
- 本项目基本情况摘要：
  ...
  - **white-attack.ipynb 白盒攻击：** 基于I-FGSM算法，$\epsilon$ 取 $0.02$ ，攻击成功率 $14.37\%$
  - **black-attack.ipynb 黑盒攻击：** 对课程CNN模型采用样本迁移和MCMC采样攻击，攻击成功率分别为 $7.9\%$ 和 $10.4\%$ ；对项目ResNet模型采用MCMC采样攻击攻击，攻击成功率为 $9.73\%$
  - **adversarial-train.ipynb 对抗训练：** 对项目初始ResNet模型在训练集上进行白盒攻击获取对抗样本，扩充训练集重新训练得到新分类器，新模型在测试集上的精度为 $\%$ ，白盒攻击
- 其他项目支撑文件：
  - ResNet34_draft.ipynb 尝试不同的超参数和数据预处理方式训练分类器模型
  - model.py 课程CNN模型
  - fmnist_dataset.py 课程数据读入文件
...
以下为部分攻击成功的样本图片，图片两两一组，左侧的为原始图像，右侧的为白盒攻击图像。可见 $\epsilon$ 取 $0.02$ 白盒攻击对图像的改动很微小，而且主要体现为图像亮度/对比度的变化，使得分类器模型错判。例如在运动鞋->袋子的定向攻击上，白盒攻击消除了鞋子部分的局部纹理特征，使分类器失效。
...
- 随机抽取训练集上8000个被正确分类的样本，利用白盒攻击获取对抗样本，最终获得 个对抗样本，在训练集上的攻击成功率为 $\%$
...
- 将对抗样本与原始训练集合并作为新的训练集，和之前保持所有训练参数一致，重新训练一个新的分类器。
- 对新的分类器进行测试，比较其在旧分类器上的提升。

新分类器的训练情况以及其在测试集上的混淆矩阵如下：
...
分类器 | 训练集精度 | 测试集精度 | 白盒攻击成功率 | 黑盒攻击成功率
---------|----------|---------|---------|---------|
 原始分类器 | B1 | C1 |
 新分类器（增加对抗样本训练） | B2 | C2 | | |